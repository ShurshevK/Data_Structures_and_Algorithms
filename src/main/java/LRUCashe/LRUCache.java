...
public class LRUCache {
    ...
    public void put (int id, String data){
        ...
        //we have to insert int othet cache + set it to be the head node
    ...
    private void removeTail() {
        ...
        // set the next node to be a Null: because its the right-most item
    ...
    public Node get(int id){
        ...
        // the map does not contain the item [0(1)] running time!!!
    ...
    // move the given nde to the front (head)
    private void update(Node node) {
        ...
        else  // we know that is the head (first node)
            this.linkedList.setHeadNode(nextNode);

        // so not the last node
        if(nextNode!=null)
            nextNode.setPreviousNode(previousNode);
        else  // we know that is the last node
