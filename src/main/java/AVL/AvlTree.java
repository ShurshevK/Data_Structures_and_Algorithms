...
public class AvlTree implements Tree {
    ...
    private Node rightRotation(Node node) {
        ...
        Node tempLeftChild = node.getLeftNode();
        Node grandChild = tempLeftChild.getRightNode();

        tempLeftChild.setRightNode(node);
        node.setLeftNode(grandChild);

        node.setHeight(Math.max(height(node.getLeftNode()), height(node.getRightNode())) + 1);
        tempLeftChild.setHeight(Math.max(height(tempLeftChild.getLeftNode()), height(tempLeftChild.getRightNode())) + 1);
